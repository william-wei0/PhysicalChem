...
function SceneGrid() {
  const grid = useMemo(() => {
    const g = new THREE.GridHelper(500, 50, 0x000000, 0x6f6f6f);
  ...
  return (
    <>
      ...
      <group ref={atom1GroupRef}>
        <mesh>
          <sphereGeometry args={[atom1Config.size, 32, 16]} />
          <meshBasicMaterial color={atom1Config.color} />
        </mesh>
      ...
      <group ref={atom2GroupRef}>
        <mesh>
          <sphereGeometry args={[atom2Config.size, 32, 16]} />
          <meshBasicMaterial color={atom2Config.color} />
        </mesh>
...
export default function RigidRotorSimulation2() {
  ...
  const [atom1Color, setAtom1Color] = useState("#003cff");
  const [atom1OutlineColor, setAtom1OutlineColor] = useState("#ffffff");
  const [atom1Outline, setAtom1Outline] = useState(true);
  const [atom1Size, setAtom1Size] = useState([7]);

  const [atom2Color, setAtom2Color] = useState("#003cff");
  const [atom2OutlineColor, setAtom2OutlineColor] = useState("#ffffff");
  const [atom2Outline, setAtom2Outline] = useState(true);
